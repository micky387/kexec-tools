...
static uint32_t apq8084_dtb_compatible(void *dtb, struct msm_id *devid, struct msm_id *dtb_id)
{
    int root_offset;
    const void *prop;
    int len;
...
static int apq8084_choose_dtb(const char *dtb_img, off_t dtb_len, char **dtb_buf, off_t *dtb_length)
{
    ...
    while(dtb + sizeof(struct fdt_header) < dtb_end)
    {
        ...
        if(apq8084_dtb_compatible(dtb, &devid, &dtb_id))
        {
...
static int apq8084_copy_prop(void *dtb_buf, const char *node, const char *name)
{
    char buf[256];
    FILE *f;
    int res = -1;
...
static int apq8084_add_extra_regs(void *dtb_buf)
{
    DIR *d;
    struct dirent *dt;
    int res = -1;

    if(apq8084_copy_prop(dtb_buf, "/memory", "reg") < 0)
        return -1;
    ...
    while((dt = readdir(d)))
    {
        ...
        if(apq8084_copy_prop(dtb_buf, "/chosen", dt->d_name) < 0)
            goto exit;
    }
